...
We also wanted to examine the relationship between the predictors to determine if there is value in including interaction effects. To do so,
we plot boxplots for 
...
$$
\begin{aligned} \quad Y &= \beta_0 + \beta_1log(Stokes) + \beta_2{log(Stokes)^2} + \beta_3{log(Stokes)^3} + \beta_4{log(Stokes)^4} + \beta_5Reynolds \\&\quad + \beta_6Froud + \beta_7(log(Stokes) * Froud) + \beta_8(log(Stokes) * Reynolds) + \beta_9(Froud * Reynolds) + \epsilon \\\text{Where:} \\Y & : \text{Moment} \\\beta_0 & : \text{Intercept} \\\beta_1 & : \text{Coefficient of log of Stokes Number} \\\beta_2 & : \text{Coefficient of log of Stokes Number squared} \\\beta_3 & : \text{Coefficient of log of Stokes Number cubed} \\\beta_4 & : \text{Coefficient of log of Stokes Number tesseracted} \\\beta_5 & : \text{Coefficient of Reynolds Number} \\\beta_6 & : \text{Coefficient of Froud Number} \\\beta_7 & : \text{Coefficient of Interaction Term of log Stokes Number and Froud Number } \\\beta_8 & : \text{Coefficient of Interaction Term of log Stokes Number and Reynolds Number} \\\beta_9 & : \text{Coefficient of Interaction Term of Froud Number and Reynolds Number} \\\epsilon & : \text{Random error}\end{aligned}
$$
...
model_df <- as.data.frame(coef(summary(firstreg)))
model_df <- model_df %>%
